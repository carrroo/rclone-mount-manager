...
pub use detect::find_rclone;
pub use mount::{do_mount, do_unmount};

use crate::error::AppError;
use serde::{Deserialize, Serialize};

/// A single mount item (parsed from rclone.conf).
...
impl RcloneManager {
    ...
    pub fn get_all_mounts(&self) -> Result<Vec<MountItem>, String> {
    ...
    pub fn update_remote_config(&self, name: &str, updates: std::collections::HashMap<String, String>) -> Result<(), String> {
        let rclone_path = self.resolve_rclone_path()?;
        update_remote_config(&self.config_path, &rclone_path, name, updates)
    }
